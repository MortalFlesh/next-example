...
const validateRequired = validate => text =>
  text.length === 0 ? { type: 'required' } : validate(text);

const validateTrimAndRequired = validate =>
  validateTrim(validateRequired(validate));

const validateEmail = validateTrimAndRequired(email => {
  if (!isEmail(email)) return { type: 'email' };
});

const validatePassword = validateTrimAndRequired(password => {
  if (password.length < 6) return { type: 'minLength', minLength: 6 };
  if (password.length > 1024) return { type: 'maxLength', maxLength: 1024 };
});

const validateShortText = validateTrimAndRequired(shortText => {
